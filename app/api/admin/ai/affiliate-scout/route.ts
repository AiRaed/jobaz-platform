...
import { generateAffiliateScoutReport } from '@/lib/admin/ai/affiliateScout'

export const dynamic = 'force-dynamic'

const ALLOWED_MODES = ['suggest', 'tasks', 'missing', 'prioritise'] as const

/**
 * Generate Affiliate Scout report.
 * POST /api/admin/ai/affiliate-scout
 */
export async function POST(req: NextRequest) {
  ...
  try {
    ...
    const result = await generateAffiliateScoutReport({
      mode,
    ...
    return NextResponse.json(
      { ok: false, error: e instanceof Error ? e.message : 'Failed to generate affiliate scout report' },
      { status: 500 }
    )
  }
}
