/**
 * Shared helpers for Arts, Media & Creative Industries Career Knowledge Library.
 * Stage model: arts_media_creative_route
 */
...
export const ARTS_STAGE_KEYS: ArtsStageKey[] = [
  'foundation_creative_support',
  'graduate_creative_entry',
  'junior_creative_professional',
  'experienced_creative',
...
export const FIELD_SLUG = 'arts-media-creative-industries'
export const MODEL_KEY = 'arts_media_creative_route'

export type RoleSeed = {
  name: string
  description: string
  stageKey: ArtsStageKey
...
> = {
  foundation_creative_support: {
    ...
    experienceRequirementLabel:
      'School-leaver, college or vocational entry; portfolio/experience may help; degree not required',
  ...
  graduate_creative_entry: {
    ...
    minimumExperienceYears: 0,
    experienceRequirementLabel:
      'Relevant degree and/or strong portfolio commonly expected for graduate creative entry',
    fitClassification: 'immediate',
  ...
  junior_creative_professional: {
    ...
    experienceRequirementLabel: 'Typically 0–2 years delivering creative briefs under supervision',
    fitClassification: 'immediate',
    academicRequirement: 'degree_relevant',
  },
  experienced_creative: {
    ...
    experienceRequirementLabel:
      'Typically 3+ years with independent brief ownership; Master’s not automatic seniority',
  ...
  senior_specialist: {
    ...
    experienceRequirementLabel:
      'Typically 5+ years with deep craft expertise and mentoring responsibility',
    fitClassification: 'realistic_next',
    academicRequirement: 'degree_relevant',
  },
  creative_leadership: {
    ...
    experienceRequirementLabel:
      'Substantial creative delivery plus team/lead responsibility; future progression for juniors',
  ...
  executive_studio_director: {
    ...
    experienceRequirementLabel:
      'Extensive creative leadership; studio director / creative director / EP — future progression only',
    fitClassification: 'future_progression',
    academicRequirement: 'degree_relevant',
  },
  academic_research: {
    ...
    minimumExperienceYears: 0,
    experienceRequirementLabel:
      'Doctorate typically required for academic tracks; research/practice-based PhD pathways included',
...
export function r(
  ...
  } else if (stageKey === 'executive_studio_director' || stageKey === 'creative_leadership') {
    defaultCategory = 'leadership'
  } else if (stageKey === 'senior_specialist') {
    defaultCategory = 'technical_specialist'
  } else if (
    stageKey === 'foundation_creative_support' ||
    stageKey === 'graduate_creative_entry'
  ) {
    defaultCategory = 'graduate_entry'
  } else if (stageKey === 'junior_creative_professional' || /design/i.test(name)) {
    defaultCategory = /design/i.test(name) ? 'design' : 'professional_practice'
  }
...
const STAGES_DEF: Array<{
  stage_key: ArtsStageKey
  label: string
  description: string
  sort_order: number
}> = [
  {
    stage_key: 'foundation_creative_support',
    label: 'Foundation / Creative Support',
    description: 'Creative support and studio/production entry roles.',
    sort_order: 10,
  },
  {
    stage_key: 'graduate_creative_entry',
    label: 'Graduate / Creative Entry',
    description: 'Graduate and first creative professional roles.',
    sort_order: 20,
  },
  {
    stage_key: 'junior_creative_professional',
    label: 'Junior Creative Professional',
    description: 'Early-career creative practitioners.',
    sort_order: 30,
  },
  {
    stage_key: 'experienced_creative',
    label: 'Experienced Creative',
    description: 'Experienced creative practitioners.',
    sort_order: 40,
  },
  {
    stage_key: 'senior_specialist',
    label: 'Senior Specialist',
    description: 'Senior creative specialists and lead craft roles.',
    sort_order: 50,
  },
  {
    stage_key: 'creative_leadership',
    label: 'Creative Leadership',
    description: 'Creative leads and heads of craft.',
    sort_order: 60,
  },
  {
    stage_key: 'executive_studio_director',
    label: 'Executive / Studio Director',
    description: 'Creative directors, studio directors and executive producers.',
    sort_order: 70,
  },
  {
    stage_key: 'academic_research',
    label: 'Academic / Research',
    description: 'Academic and research careers in arts and media.',
    sort_order: 80,
  },
]

export async function ensureArtsStageModel(supabase: SupabaseClient): Promise<{
  modelId: string
  ...
  if (existing?.id) {
    ...
    await supabase
      .from('career_library_stage_models')
      .update({
        name: 'Arts, media & creative industries route',
        description:
          'UK arts, media and creative industries progression by portfolio, experience and creative responsibility. Master\'s/PhD are not automatic seniority.',
        active: true,
        is_system: true,
        sort_order: 120,
      })
    ...
    const { data: created, error } = await supabase
      ...
      .insert({
        ...
        name: 'Arts, media & creative industries route',
        description:
          'UK arts, media and creative industries progression by portfolio, experience and creative responsibility.',
        active: true,
        sort_order: 120,
        is_system: true,
      })
  ...
  for (const key of ARTS_STAGE_KEYS) {
...
export async function populateArtsPack(
  supabase: SupabaseClient,
  pack: SpecialismPack,
  stageByKey: Map<string, { id: string; stage_key: string; label: string }>,
  reservedNames: Set<string>,
  ...
  const byStage = new Map<ArtsStageKey, RoleSeed[]>()
  for (const key of ARTS_STAGE_KEYS) byStage.set(key, [])
...
export async function runArtsPopulate(packs: SpecialismPack[], title: string) {
  const supabase = createServiceClient()
  const { modelId, stageByKey } = await ensureArtsStageModel(supabase)
  ...
  if (!field?.id) {
    throw new Error('Arts, Media & Creative Industries field not found. Run seed script first.')
  }
  ...
  for (const pack of packs) {
    const result = await populateArtsPack(
      supabase,
      pack,
      stageByKey,
      reservedNames,
