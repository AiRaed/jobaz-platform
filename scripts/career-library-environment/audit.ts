/**
 * Full audit for Environment, Agriculture & Food library.
 *   npx tsx scripts/career-library-environment/audit.ts
 */

import { createServiceClient, EAF_STAGE_KEYS, FIELD_SLUG, MODEL_KEY } from './shared'
...
async function main() {
  ...
  for (const k of EAF_STAGE_KEYS) byStage[k] = 0
  let nonDegree = 0
  let masters = 0
  let phd = 0
  ...
  console.log('=== ENVIRONMENT, AGRICULTURE & FOOD AUDIT ===\n')
  console.log(
  ...
  for (const k of EAF_STAGE_KEYS) console.log(`   ${k}: ${byStage[k]}`)
  console.log(`7. Non-degree-accessible: ${nonDegree}`)
  console.log(`8. Master’s-relevant: ${masters}`)
  console.log(`9. PhD-relevant: ${phd}`)
  console.log(`10. Duplicate titles: ${overlaps.length}`)
  console.log('11. Bodies (sample):')
  for (const b of [...bodies].sort().slice(0, 30)) console.log(`   - ${b}`)
  if (bodies.size > 30) console.log(`   ... +${bodies.size - 30} more`)
  ...
  console.log(
    `All stage keys present: ${EAF_STAGE_KEYS.every((k) =>
