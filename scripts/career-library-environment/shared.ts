/**
 * Shared helpers for Environment, Agriculture & Food Career Knowledge Library.
 * Stage model: environment_agriculture_food_route
 */
...
export const EAF_STAGE_KEYS: EafStageKey[] = [
  'foundation_technical_support',
  'graduate_entry',
  'professional_practitioner',
  'experienced_specialist',
...
export const FIELD_SLUG = 'environment-agriculture-food'
export const MODEL_KEY = 'environment_agriculture_food_route'

export type RoleSeed = {
  name: string
  description: string
  stageKey: EafStageKey
...
> = {
  foundation_technical_support: {
    ...
    experienceRequirementLabel:
      'School-leaver, college or vocational entry; degree not required for many technical support roles',
  ...
  graduate_entry: {
    ...
    minimumExperienceYears: 0,
    experienceRequirementLabel:
      'Relevant degree commonly expected for graduate schemes and first professional roles',
    fitClassification: 'immediate',
  ...
  professional_practitioner: {
    ...
    experienceRequirementLabel:
      'Typically 1–3 years in agriculture, food, environment, forestry or rural land practice',
  ...
  experienced_specialist: {
    ...
    experienceRequirementLabel:
      'Typically 4+ years with independent site/project ownership; Master’s may help niches but is not automatic seniority',
    fitClassification: 'realistic_next',
    academicRequirement: 'degree_relevant',
  },
  senior_specialist: {
    ...
    experienceRequirementLabel:
      'Typically 6+ years with complex technical remit and mentoring responsibility',
  ...
  leadership_operations: {
    ...
    experienceRequirementLabel:
      'Substantial specialist experience plus farm/site/operations or programme leadership',
    fitClassification: 'future_progression',
    academicRequirement: 'degree_relevant',
  },
  executive_director: {
...
export function r(
  ...
  } else if (stageKey === 'executive_director' || stageKey === 'leadership_operations') {
  ...
  } else if (stageKey === 'foundation_technical_support' || stageKey === 'graduate_entry') {
    defaultCategory = 'graduate_entry'
  } else if (/site|farm|field|estate/i.test(name)) {
...
const STAGES_DEF: Array<{
  stage_key: EafStageKey
  label: string
  description: string
  sort_order: number
}> = [
  {
    stage_key: 'foundation_technical_support',
    label: 'Foundation / Technical Support',
    description: 'Technical support in farm, food, field and estate settings.',
    sort_order: 10,
  },
  {
    stage_key: 'graduate_entry',
    label: 'Graduate Entry',
    description: 'Graduate trainees and first professional roles.',
    sort_order: 20,
  },
  {
    stage_key: 'professional_practitioner',
    label: 'Professional Practitioner',
    description: 'Competent practitioners across EAF domains.',
    sort_order: 30,
  },
  {
  ...
  {
    stage_key: 'leadership_operations',
    label: 'Leadership / Operations',
    description: 'Farm, site and operations leadership.',
    sort_order: 60,
  },
  {
...
export async function ensureEafStageModel(supabase: SupabaseClient): Promise<{
  modelId: string
  ...
  if (existing?.id) {
    ...
    await supabase
      .from('career_library_stage_models')
      .update({
        name: 'Environment, agriculture & food route',
        description:
          'UK environment, agriculture and food progression. Master\'s/PhD are not automatic seniority outside HE/research.',
        active: true,
        is_system: true,
        sort_order: 160,
      })
    ...
    const { data: created, error } = await supabase
      ...
      .insert({
        ...
        name: 'Environment, agriculture & food route',
        description:
          'UK environment, agriculture and food progression by experience and responsibility.',
        active: true,
        sort_order: 160,
        is_system: true,
      })
  ...
  for (const key of EAF_STAGE_KEYS) {
...
export async function populateEafPack(
  supabase: SupabaseClient,
  pack: SpecialismPack,
  stageByKey: Map<string, { id: string; stage_key: string; label: string }>,
  reservedNames: Set<string>,
  ...
  const byStage = new Map<EafStageKey, RoleSeed[]>()
  for (const key of EAF_STAGE_KEYS) byStage.set(key, [])
...
export async function runEafPopulate(packs: SpecialismPack[], title: string) {
  const supabase = createServiceClient()
  const { modelId, stageByKey } = await ensureEafStageModel(supabase)
  ...
  if (!field?.id) {
    throw new Error('Environment, Agriculture & Food field not found. Run seed script first.')
  }
  ...
  for (const pack of packs) {
    const result = await populateEafPack(
      supabase,
      pack,
      stageByKey,
      reservedNames,
