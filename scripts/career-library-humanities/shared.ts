/**
 * Shared helpers for Humanities & Social Sciences Career Knowledge Library.
 * Stage model: humanities_social_sciences_route
 */
...
export const HSS_STAGE_KEYS: HssStageKey[] = [
  'foundation_social_support',
  'graduate_social_sciences_entry',
  'professional_practitioner',
  'experienced_specialist',
...
export const FIELD_SLUG = 'humanities-social-sciences'
export const MODEL_KEY = 'humanities_social_sciences_route'

export type RoleSeed = {
  name: string
  description: string
  stageKey: HssStageKey
...
> = {
  foundation_social_support: {
    ...
    experienceRequirementLabel:
      'School-leaver, college or vocational entry; degree not required; no professional registration required',
  ...
  graduate_social_sciences_entry: {
    ...
    minimumExperienceYears: 0,
    experienceRequirementLabel:
      'Relevant degree commonly expected; professional registration not required at graduate entry',
    fitClassification: 'immediate',
  ...
  professional_practitioner: {
    ...
    experienceRequirementLabel:
      'Typically 1–3 years in research, policy, heritage, culture or applied social science roles',
  ...
  experienced_specialist: {
    ...
    experienceRequirementLabel:
      'Typically 4+ years with independent project ownership; Master’s may help niches but is not automatic seniority',
    fitClassification: 'realistic_next',
    academicRequirement: 'degree_relevant',
  },
  senior_specialist: {
    ...
    experienceRequirementLabel:
      'Typically 6+ years with complex remit and mentoring responsibility',
  ...
  leadership: {
    ...
    experienceRequirementLabel:
      'Substantial specialist experience plus team or programme leadership',
    fitClassification: 'future_progression',
    academicRequirement: 'degree_relevant',
  },
  executive_director: {
    ...
    experienceRequirementLabel:
      'Extensive leadership; director / head of function — future progression only',
    fitClassification: 'future_progression',
  ...
  academic_research: {
    ...
    minimumExperienceYears: 0,
    experienceRequirementLabel:
      'Doctorate typically required for academic tracks; separate from professional practice seniority',
...
export function r(
  ...
  } else if (stageKey === 'executive_director' || stageKey === 'leadership') {
    defaultCategory = 'leadership'
  } else if (stageKey === 'senior_specialist' || stageKey === 'experienced_specialist') {
  ...
  // Graduate entry and foundation never require registration unless explicitly overridden
  const defaultReg: CareerLibraryRegistrationRequirement =
    stageKey === 'foundation_social_support' || stageKey === 'graduate_social_sciences_entry'
      ? 'none'
      : 'none'
  ...
  return {
    ...
    professionalRegistrationRequirement: opts.professionalRegistrationRequirement ?? defaultReg,
...
const STAGES_DEF: Array<{
  stage_key: HssStageKey
  label: string
  description: string
  sort_order: number
}> = [
  {
    stage_key: 'foundation_social_support',
    label: 'Foundation / Social Support',
    description: 'Social, research and cultural support roles.',
    sort_order: 10,
  },
  {
    stage_key: 'graduate_social_sciences_entry',
    label: 'Graduate / Social Sciences Entry',
    description: 'Graduate entry — no professional registration required.',
    sort_order: 20,
  },
  {
    stage_key: 'professional_practitioner',
    label: 'Professional Practitioner',
    description: 'Professional practitioners in humanities and social sciences.',
    sort_order: 30,
  },
  {
    stage_key: 'experienced_specialist',
    label: 'Experienced Specialist',
    description: 'Experienced specialists with independent ownership.',
    sort_order: 40,
  },
  {
    stage_key: 'senior_specialist',
    label: 'Senior Specialist',
    description: 'Senior specialists and principal practitioners.',
    sort_order: 50,
  },
  {
    stage_key: 'leadership',
    label: 'Leadership',
    description: 'Team and programme leadership.',
    sort_order: 60,
  },
  {
    stage_key: 'executive_director',
    label: 'Executive / Director',
    description: 'Directors and heads of function.',
    sort_order: 70,
  },
  {
...
export async function ensureHssStageModel(supabase: SupabaseClient): Promise<{
  modelId: string
  ...
  if (existing?.id) {
    ...
    await supabase
      .from('career_library_stage_models')
      .update({
        name: 'Humanities & social sciences route',
        description:
          'UK humanities and social sciences progression. Graduate entry does not require professional registration. Master\'s/PhD are not automatic seniority outside HE/research.',
        active: true,
        is_system: true,
        sort_order: 150,
      })
    ...
    const { data: created, error } = await supabase
      ...
      .insert({
        ...
        name: 'Humanities & social sciences route',
        description:
          'UK humanities and social sciences progression by experience and responsibility.',
        active: true,
        sort_order: 150,
        is_system: true,
      })
  ...
  for (const key of HSS_STAGE_KEYS) {
...
export async function populateHssPack(
  supabase: SupabaseClient,
  pack: SpecialismPack,
  stageByKey: Map<string, { id: string; stage_key: string; label: string }>,
  reservedNames: Set<string>,
  ...
  const byStage = new Map<HssStageKey, RoleSeed[]>()
  for (const key of HSS_STAGE_KEYS) byStage.set(key, [])
...
export async function runHssPopulate(packs: SpecialismPack[], title: string) {
  const supabase = createServiceClient()
  const { modelId, stageByKey } = await ensureHssStageModel(supabase)
  ...
  if (!field?.id) {
    throw new Error('Humanities & Social Sciences field not found. Run seed script first.')
  }
  ...
  for (const pack of packs) {
    const result = await populateHssPack(
      supabase,
      pack,
      stageByKey,
      reservedNames,
