/**
 * Full audit for Languages & Literature library.
 *   npx tsx scripts/career-library-languages/audit.ts
 */

import { createServiceClient, LANG_STAGE_KEYS, FIELD_SLUG, MODEL_KEY } from './shared'
...
async function main() {
  ...
  for (const spec of specs ?? []) {
    ...
    for (const role of roles ?? []) {
      ...
      if (stageKey === 'foundation_language_support') foundation += 1
      if (stageKey === 'graduate_language_entry') graduate += 1
      if (stageKey === 'professional_linguist') professional += 1
      if (stageKey === 'experienced_specialist') experienced += 1
      if (stageKey === 'senior_specialist') senior += 1
      if (stageKey === 'leadership') leadership += 1
      if (stageKey === 'executive_director') executive += 1
  ...
  console.log('=== LANGUAGES & LITERATURE AUDIT ===\n')
  console.log(
  ...
  console.log(`6. Foundation / language support: ${foundation}`)
  console.log(`7. Graduate / language entry: ${graduate}`)
  console.log(`8. Professional linguist: ${professional}`)
  console.log(`9. Experienced specialist: ${experienced}`)
  console.log(`10. Senior specialist: ${senior}`)
  console.log(`11. Leadership: ${leadership}`)
  console.log(`12. Executive / director: ${executive}`)
  ...
  console.log(
    `All stage keys present: ${LANG_STAGE_KEYS.every((k) =>
