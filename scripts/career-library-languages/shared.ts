/**
 * Shared helpers for Languages & Literature Career Knowledge Library.
 * Stage model: languages_literature_route
 */
...
export const LANG_STAGE_KEYS: LangStageKey[] = [
  'foundation_language_support',
  'graduate_language_entry',
  'professional_linguist',
  'experienced_specialist',
...
export const FIELD_SLUG = 'languages-literature'
export const MODEL_KEY = 'languages_literature_route'

export type RoleSeed = {
  name: string
  description: string
  stageKey: LangStageKey
...
> = {
  ...
  graduate_language_entry: {
    ...
    minimumExperienceYears: 0,
    experienceRequirementLabel:
      'Relevant language/literature degree commonly expected; CELTA/CertTESOL may apply for English teaching entry',
    fitClassification: 'immediate',
  ...
  professional_linguist: {
    ...
    experienceRequirementLabel:
      'Typically 1–3 years practising as a linguist, language teacher, translator or literary practitioner',
  ...
  experienced_specialist: {
    ...
    experienceRequirementLabel:
      'Typically 4+ years with independent assignment ownership; Master’s may help niches but is not automatic seniority',
    fitClassification: 'realistic_next',
    academicRequirement: 'degree_relevant',
  },
  senior_specialist: {
  ...
  executive_director: {
    ...
    experienceRequirementLabel:
      'Extensive leadership; director / head of languages — future progression only',
    fitClassification: 'future_progression',
...
export function r(
  ...
  if (stageKey === 'academic_research') {
    ...
    stageKey === 'foundation_language_support' ||
    stageKey === 'graduate_language_entry'
  ) {
...
const STAGES_DEF: Array<{
  stage_key: LangStageKey
  label: string
  description: string
  sort_order: number
}> = [
  {
    stage_key: 'foundation_language_support',
    label: 'Foundation / Language Support',
    description: 'Language and literary programme support roles.',
    sort_order: 10,
  },
  {
    stage_key: 'graduate_language_entry',
    label: 'Graduate / Language Entry',
    description: 'Graduate language and literature entry roles.',
    sort_order: 20,
  },
  {
    stage_key: 'professional_linguist',
    label: 'Professional Linguist',
    description: 'Practising linguists and language professionals.',
    sort_order: 30,
  },
  {
    stage_key: 'experienced_specialist',
    label: 'Experienced Specialist',
    description: 'Experienced language and literature specialists.',
    sort_order: 40,
  },
  {
    stage_key: 'senior_specialist',
    label: 'Senior Specialist',
    description: 'Senior language and literature specialists.',
    sort_order: 50,
  },
  {
    stage_key: 'leadership',
    label: 'Leadership',
    description: 'Language programme and service leadership.',
    sort_order: 60,
  },
  {
    stage_key: 'executive_director',
    label: 'Executive / Director',
    description: 'Directors and heads of languages.',
    sort_order: 70,
  },
  {
    stage_key: 'academic_research',
    label: 'Academic / Research',
    description: 'University teaching and research in languages and literature.',
    sort_order: 80,
  },
]

export async function ensureLangStageModel(supabase: SupabaseClient): Promise<{
  modelId: string
  ...
  if (existing?.id) {
    ...
    await supabase
      .from('career_library_stage_models')
      .update({
        name: 'Languages & literature route',
        description:
          'UK languages and literature progression by language competence, professional practice and academic research. Master\'s/PhD are not automatic seniority outside HE/research.',
        active: true,
        is_system: true,
        sort_order: 140,
      })
    ...
    const { data: created, error } = await supabase
      ...
      .insert({
        ...
        name: 'Languages & literature route',
        description:
          'UK languages and literature progression by language competence, professional practice and academic research.',
        active: true,
        sort_order: 140,
        is_system: true,
      })
  ...
  for (const key of LANG_STAGE_KEYS) {
  ...
  for (const seed of roles) {
    ...
    const { error } = await supabase.from('career_library_roles').insert({
      ...
      metadata: {
        ...
        languages_stage: stage.stage_key,
...
export async function populateLangPack(
  supabase: SupabaseClient,
  pack: SpecialismPack,
  stageByKey: Map<string, { id: string; stage_key: string; label: string }>,
  reservedNames: Set<string>,
  ...
  const byStage = new Map<LangStageKey, RoleSeed[]>()
  for (const key of LANG_STAGE_KEYS) byStage.set(key, [])
...
export async function runLangPopulate(packs: SpecialismPack[], title: string) {
  const supabase = createServiceClient()
  const { modelId, stageByKey } = await ensureLangStageModel(supabase)
  ...
  if (!field?.id) {
    throw new Error('Languages & Literature field not found. Run seed script first.')
  }
  ...
  for (const pack of packs) {
    const result = await populateLangPack(
      supabase,
      pack,
      stageByKey,
      reservedNames,
